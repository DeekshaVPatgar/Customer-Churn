...
Select* from capstone.customer_churn;
...
select max(monthly_charges), min(monthly_charges), round(avg(monthly_charges),2), round(stddev(monthly_charges),2)
from customer_churn
where churn_status = 'yes';
...
when contract_type = 'monthly' then count(1)
...
-- 17.	Calculate the average age and total charges for customers with different combinations of streaming services
 select avg(age),round(sum(total_charges),2),streaming_tv,streaming_movies,count(customer_id)
...
select avg(age),round(sum(total_charges),2),internet_service,phone_service
from customer_churn where churn_status = 'yes'
...
-- 26.	Create a view to find the customers who have churned and their cumulative total charges over time
  select customer_id,sum(total_charges) from customer_churn
  group by customer_id;
